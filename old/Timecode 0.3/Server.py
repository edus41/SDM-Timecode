...
class Server(Process):
    def __init__(self, host="127.0.0.1", port=12345, message=None, status=None):
        super().__init__()
        self.host = host
        self.port = port
        ...
        self.revice_queue = message
        self.send_queue = status
    ...
    def _conect(self):
        while not self.online:
            ...
            try:
                ...
                self.online = True
                self.send_queue.put(self.online)
                ...
                if e.errno == 10048:
                    log(
                        f"[SERVER ERROR]: Ya Existe Un Servidor Iniciado en: {self.direccion}, Reintentando...",
                        RED,
                    )
                elif e.errno == 10038:
    ...
    def _sender(self, conexion, address):
        while self.online:
            if self.stop_event.is_set():
                conexion.close()
                break
            if not self.revice_queue.empty():
                try:
                    data_str = str(self.revice_queue.get())
